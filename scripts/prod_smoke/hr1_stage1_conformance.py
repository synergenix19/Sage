...
AR = re.compile(r"[؀-ۿ]")
AUDIT_COLUMNS = ("COALESCE(skill_match_method,'<none>'), COALESCE(node_path::text,''), "
...
def sid(tag):
    return prod_probe.new_session_id(f"prodsuite-hrconf-{tag}")


def chat(s, text):
    if s not in _SIDS:
        _SIDS.append(s)
    return prod_probe.chat(s, text, base_url=URL, api_key=KEY, user_id=TEST_USER, timeout=80).message
