...
class SageState(TypedDict):
    ...
    embedding_timeout: Optional[bool]   # F4: True when skill_select's BGE-M3 embed timed out (keyword-only fallback this turn); per-turn reset by _build_state — audit/degradation consumers read it from state, was silently dropped while undeclared
    ...
    opener_rewrite: Optional[dict]         # #58 traceability: {"applied": bool, "model": str, "opener": str, "latency_ms": int, "suppressed": Optional[str]} or None when the opener-rewrite allowlist gate never ran this turn. Set by output_gate's opener-rewrite logic and included unconditionally in output_gate_node's returned dict every turn it runs (same per-turn discipline as hr_neutrality_rejected / banned_opener_violation immediately above — no explicit _build_state reset needed, since output_gate overwrites it fresh whenever it runs; absent/stale on turns that bypass output_gate entirely, e.g. crisis/medical/high_risk/derealization/screen terminals). Declared channel (LangGraph drops undeclared keys between nodes — the SG-2 seam class); was previously computed and returned by output_gate.py but silently dropped because this key was never declared here.
