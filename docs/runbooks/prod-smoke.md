...
>
> Scripts on `scripts/lib/prod_probe.py`'s shared `resolve_creds()` (the migrating prod-HTTP drivers): the `railway variables --json` fallback is opt-in only. Interactive operators who relied on it firing implicitly must now either export `SAGE_API_KEY`/`DATABASE_URL` themselves or set `SAGE_ALLOW_RAILWAY_FALLBACK=1` — otherwise `resolve_creds()` raises instead of silently pulling live prod credentials.
