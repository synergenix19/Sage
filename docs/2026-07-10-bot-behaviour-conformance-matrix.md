...
**Safety-copy DoD (2026-07-10 lesson — SG-2):** JSON-presence / content-contract tests are NECESSARY BUT NEVER SUFFICIENT for safety copy. **A green content-contract test is explicitly NOT evidence that the copy fires** — SG-2 was green on the JSON while the caveat never reached the user (LLM-discretionary delivery) until the gate-injection fix. Every **safety-copy** row's DoD is therefore **content present + behavioral firing test + driven transcript** (three parts, not two). This is the third *per-layer-green-masks-end-to-end-miss* incident class, after #191 (render seam) and #205 (affordance seam).
...
- **SG-2 → GAP (content deployed, does NOT fire)** — the cardiac/pregnancy caveat is LLM-discretionary (entry_screen "Example approach"); `freeflow_respond` drops it and suggests cold water WITHOUT the screen across 4 EN + 1 AR drives. Needs a **deterministic firing fix** (force-inject before any temperature/exercise instruction) — Contraindication-Firing-Gap class. **The unit tests passed on JSON but the driven test caught the non-firing — this is why CONFORMS requires a transcript, not a code read.** **FIX BUILT (PR #298):** deterministic `mandatory_caveat` gate injection (`_pin_contraindication_caveat`, Cardinal-Rule class fix) — status now **GAP-DEPLOYED** ("content live, protection absent") → CODED + firing-test-green → CONFORMS pending the safety-exception deploy + Ring-1/2.2 re-run (coordinated with H4 values).
