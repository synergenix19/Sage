...
**Decision (2026-07-10, PO):** ☑ **adopt the doc's list exactly, config-driven.** ⚠ **REVERSES GL-1** — superseded; doc is normative. **ALL GATES CLEARED 2026-07-10:** ☑ reversal confirmed · ☑ freeze lifted · ☑ clinical sign-off (Vee) · ☑ **dial-test CONFIRMED (all 5 numbers)**. → **GO for build + coordinated safety deploy** (with SG-2 firing fix; Ring-1 + 2.2 re-run). **COUPLING:** National `800-4673` is **8am–8pm**, so the value change must ship WITH the multi-resource, hours-aware card — a lone 8am–8pm number strands a night user. H4 values are coupled to crisis-card Phase 1.
