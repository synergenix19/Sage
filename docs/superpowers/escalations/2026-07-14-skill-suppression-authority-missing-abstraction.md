...
**Exhibit — the artifact-check that let the worst string through (2026-07-15).** The medical terminal shipped instructing a cardiac patient to call **999 (UAE police)** instead of **998 (ambulance)**, and it was reported "resolved" this way, verbatim:

> "Confirmed gate-2 resolved by the exact artifact-check that let it through."

The check performed was *does config match the doc?* — and it did, because the doc's Resources table (authored for the psychiatric-crisis pathway, where police co-response is appropriate) said 999, and the drafted text inherited it. Conformance to the artifact was checked; conformance to reality was not — on the **highest-consequence string in the system**, after it had been flagged three times. Not carelessness: the mechanism. This is the thread's whole lesson in one line, and it is one more exhibit that **only a mechanical gate catches this class** — the mitigation is the numbers ticket's rule (*no number leads a safety response without a documented dial-test/citation record*), not more attention.

**Exhibit — the correction asserted its own conformance to reality (same day).** The 998 fix then shipped with a comment reading *"reality-verified 2026-07-15"* — a **label on a safety claim whose verification record did not exist**. Review caught it (the crisis-number GL-1 history made it recognisable). This is the pattern at its most refined: an artifact asserting its own conformance to reality. The rule the ticket set — *no number without a record* — therefore binds the correction as tightly as the error: 998 leading the medical terminal is **unverified-by-record** until the dial-test ticket closes; a web citation is evidence, not a record. The rule holds against its own author. That is the only kind of rule worth having here.
