...
**INQ item priority (PMC10642825 network analysis, 2023):** Within the 6-item PB subscale, items 3 ("I am a burden on society") and 6 ("I make things worse for people in my life") have the strongest direct associations with SI; the remaining four items show weaker or mediated relationships. For the TB subscale, items 11 and 12 cluster most strongly with SI; the full 9-item subscale is not recommended for single-session screening. A 4-item shortlist (PB 3+6, TB 11+12) is explicitly recommended for repeated real-time monitoring contexts — exactly Sage's use case. Seed the SK-EN-002 burdensomeness/belongingness extension from these four items first before extending to the full INQ item set.

**Why phrase-by-phrase patching fails (literature evidence):** Coppersmith et al. (PMC10739607) found that trained human coders initially achieved Cohen's Kappa of 0.50 (PB) and 0.34 (TB) — below acceptable threshold — when coding burdensomeness and belongingness language in naturalistic text without supplementary construct-level training. Example texts coded as PB were highly implicit and did not contain obvious burdensomeness keywords. If trained raters require explicit construct-grounding to code the class reliably, surface-form lexical matching without construct-level authoring cannot close the gap.
...
1. **Recommended: segment-level S3 scoring** — score sentence segments individually rather than the full message as a single vector. Whole-message embedding is the identified cause of dilution; splitting on sentence boundaries and scoring each segment allows the SI-bearing clause to be evaluated on its own signal rather than diluted by a preceding emotional context clause. Architecture change, higher cost, but the architecturally correct fix. Note: direct peer-reviewed validation of segment-level vs. whole-message scoring for SI detection in a cosine-similarity architecture was not found in the literature review; this is an engineering inference, consistent with analogous findings in multi-clause emotion-region segmentation tasks but not directly citable as SI-specific evidence.
...
**Implementation constraint — delivery mode:** Published Columbia Lighthouse Project guidance requires clinician review of C-SSRS self-report results; chatbot-autonomous delivery of C-SSRS item 1 as a routing decision (without human oversight in the loop) is not validated in the peer-reviewed literature as of 2024. Any implementation of the `risk_ambiguous` → C-SSRS path must route the item 1 response to the clinician review queue rather than making an autonomous safe/crisis decision. The probe fires autonomously; the outcome decision requires a human in the loop.
