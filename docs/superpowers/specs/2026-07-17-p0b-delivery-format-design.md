...
| `staged_iterative` *(flagged — likely collapses; HER call)* | `Show all domains, then guided completion one at a time` (Life Compass, 1) | show a multi-part artifact, then complete each part iteratively | **SHARPENED question (the naive test is already-passed):** the executor ALREADY walks a guided conversation one step at a time — that is what steps ARE. So the question is NOT "does the renderer walk domains one-at-a-time" (it would, via steps) but **"does `staged_iterative` differ *behaviorally* from `visual_then_guided` delivered as a stepped conversation?"** If Life Compass = show-all-domains-then-guided-*stepped*-completion, it is already `visual_then_guided` and this value **collapses** (mint no value with no distinct consumer). Present it to her as that sharper question. Default lean: collapse. |
...
### `bare Visual` (Emotions Wheel) — DEFERRED, not shipped (its only exemplar has no skill)
`Visual` (bare, 1) has exactly one source row — Emotions Wheel (§4a) — which is on the **10-pathways-with-no-skill_id** list. Shipping `visual` day one would create an enum member with **zero library consumers** — the dormant-field pattern, in the very schema we build to prevent it. So `visual` is **NOT shipped**; it is recorded in the library-coverage finding with a named trigger: *"when Emotions Wheel lands as a skill, the enum gains `visual`."* Same deferred-extension pattern as the chain and the OR. (Checked: no other bare-`Visual` row has a library-backed skill, so nothing forces it to ship now.)

### Enum count, net
**6 shipped** (`video`, `visual_then_guided`, `guided_conversation`, `instructional`, `single_message`, `info_resource`) **+ `visual` deferred** — pending the sharpened `staged_iterative` call, which is expected to collapse (leaving 6) rather than add a 7th. The clinician ratifies the 6, rules on `staged_iterative`, and the deferred `visual` + the two chain/OR deferrals ride the library roadmap.
