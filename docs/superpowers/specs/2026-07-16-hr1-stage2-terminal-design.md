...
- **HR is the third member of the SAFETY-EXIT TERMINAL CLASS** — `crisis_response`, `medical_response`, `high_risk_response`. The class contract (stated once in the arch doc, Task 5): routed from `_route_after_safety` → straight to END, bypasses `output_gate`, writes own audit, clears skill state on entry, fixed/templated copy, precedence rank. **`low_confidence_respond` is NOT in this class** (it routes through `output_gate` like a pipeline node — my earlier "4th terminal incl. low_confidence" framing was wrong). **BOUNDARY:** the `output_gate` bypass is licensed by the copy being deterministic/templated, not by being safety-related — an LLM-rendered terminal would go through the gate. Catalogue edit rides the Stage 2 merge (proposed, human sign-off) and closes the missing `medical_response` entry.
- **Conformance — altitude correction:** Stage 1 routed HR at the post-intent altitude (below `intent_route`), a temporary non-conformance for a crisis-ranked protocol; Stage 2 corrects HR's *rank* to the pre-intent safety-exit altitude. One line into the conformance matrix. Promotion of `mania_behavior_underway` to a Node-1 flag is deferred post-CF-007-ratification (ticket `2026-07-16-hr-mania-flag-promotion.md`; promoting a read of an active:false rule now would be a #270-class inversion).
