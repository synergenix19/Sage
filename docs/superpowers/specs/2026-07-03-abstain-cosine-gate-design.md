...
### 3.8 Thin-margin watch — right-sized for POC (on-demand, not scheduled)
The margin between the threshold (0.42) and the weakest observed positive (0.4283, khaleeji/lexical) is only **~0.008 of cosine similarity**, so production paraphrases of that weakest class (short-form assertive-communication questions) can plausibly fall below 0.42 and be falsely abstained. The **durable control is the audit column itself**: migration 007 records `knowledge_top_similarity` on **every turn, permanently**, so any false abstention is retrospectively visible whenever anyone looks — nothing is lost by not polling. At POC scale (internal/pilot users, no external exposure) a falsely-abstained legitimate query also surfaces the old-fashioned way ("it wouldn't answer me" in a demo); the feedback loop is the monitor at this scale.

- **Control at POC = audit column + on-demand check.** Run `scripts/monitor_abstain_band.py` (read-only; `session_audit` abstained turns with `knowledge_top_similarity` in the **~0.40–0.44 band**) **manually before demos/pilot sessions and after any knowledge-path change**. 30 seconds, no infrastructure. Any *legitimate* in-band abstention → lever: nudge `SAGE_COSINE_ABSTAIN_THRESHOLD` down, or fail-open at `0.0` (§3.7) — a config flip, not an incident.
- **Scheduled monitoring is a PRODUCTION-HARDENING item, not a POC one.** A daily cron with prod credentials is disproportionate for a POC whose posture is minimal-change-to-production. It moves to the production-deployment checklist, bundled with the `/health/version` endpoint and the eventual LLM-observability stack (the v7 gap analysis already flags missing LLM observability as a Full Build item — this rides with it). NB: cloud-scheduler routines cannot own it regardless (no prod-DB access → a "clean" report it can't actually produce = false assurance).
