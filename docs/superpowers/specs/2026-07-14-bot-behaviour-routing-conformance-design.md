...
- **Flip blockers** (before `SAGE_MEDICAL_REDFLAG_GUARD=true`): (1) **audit columns** — *DONE 2026-07-15* (`gate_path`/`medical_flags` added conditionally to `_build_session_audit_row`, migration `012`, flag-OFF rows byte-identical; migration is a flag-flip deploy gate). (2) **998 number** — *DONE 2026-07-15* (medical terminal leads with 998=UAE ambulance, not 999=police), **with a documented dial-test/citation record as a pre-flip gate** (ticket `2026-07-15-emergency-numbers-verification`). (3) Q1-terminal ratification. (4) Q1-triggers two-part sign-off (§1 list + the two engineering variants).
- **B1-full gate limitation (Q-A, empirically confirmed 2026-07-15):** the ≥95% per-class recall gate on `medical_e3_recall.json` certifies the **phrase-class** guards only — `cardiac_stroke` (11 phrases), `sleep_disorder`, `burnout_physical`. It does **NOT** test the doc's Section 6 **judgment** guards: *"real inability to breathe rather than panic-related breathlessness," "fainting," "confusion/disorientation beyond typical panic."* Those are absent from the positive fixtures. **B1-full must either extend the fixtures with clinician-labeled positive cases for the judgment guards, or explicitly scope them out of certified coverage — a stated limitation, not a silent one** (clinical Q-A open). Related: the chest-centric list omits atypical cardiac presentation (esp. *"feels like a panic attack"*, the anxiety-product collision case) — a **doc-scope decision** (Q-B) upstream of the fixtures.
- **v7 deviations (Absolute Rule 1) — aligned to the LIVING arch doc (`SageAI_architecture_current.md`), not the stale v7 FINAL:** (a) the living doc documents a **9-node graph** (`safety_check, intent_route, skill_select, knowledge_retrieve, skill_executor, freeflow_respond, output_gate, crisis_response, low_confidence_respond` — the v7 FINAL "8 nodes" is superseded; `knowledge_retrieve` was added, and `crisis_response` is one of the 9, not a terminal outside a set). `medical_response` (B1) adds a **10th node** and must be entered in the node catalogue. *(Earlier drafts used the stale "8-node" framing — corrected here.)* (b) `medical_flags` is a **new state channel** (schema extension), distinct from `clinical_flags`, reserved by `safety_precedence.py`. Both belong in the ratified v7 record.
