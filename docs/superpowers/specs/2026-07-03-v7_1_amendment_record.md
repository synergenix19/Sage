...
> ⚠️ **TWO NAMES — set the RIGHT one.** The **environment variable is `SAGE_CRISIS_TIERING`** (`config.py`: `CRISIS_TIERING_ENABLED = os.getenv("SAGE_CRISIS_TIERING","false").lower()=="true"`). `CRISIS_TIERING_ENABLED` is the *derived Python boolean*, **not an env var** — setting `CRISIS_TIERING_ENABLED` in Railway does nothing. To flip: set **`SAGE_CRISIS_TIERING=true`** (lowercase `true`; value is `.lower()`-compared) on the correct **environment scope** (Railway scopes vars per environment — internal/staging, not prod). This dual naming is exactly how a future engineer sets the wrong variable and concludes "nothing changed, so it's safe."
