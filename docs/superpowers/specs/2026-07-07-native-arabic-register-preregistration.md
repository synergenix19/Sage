...
- **Rating:** on `gender_marked = f`/`m`, matching gendered Khaleeji address is expected and a **mis-gendered reply caps at rubric-2 and is flagged**; on `none`, neutral is expected and **not** penalized for absent gendered warmth. Marked and unmarked register are reported **separately**, so a mediocre score is interpretable (neutrality cost vs GPT-4o's Khaleeji), not conflated.
...
- **Detector caveat (measurement integrity):** `detect_gender_marking` is a starter lexicon (8 predicate-adjective pairs) with a **confirmed third-party false-positive** (a feminine mention of someone else — e.g. *أختي تعبانة* — reads as self-marking) and no Arabizi coverage. Before the marked-stratum register + mis-gender numbers are trusted: the false-positive **rate is quantified over the 431** (the detector run yields it) and the lexicon gets Gulf-native linguist review. Recommended refinement: a first-person-anchor guard (require أنا/إني/ـني near the marker), biased toward false-negatives (→ neutral) over false-positives (→ wrong gender), per "a wrong guess is worse than neutrality."
