...
Layer 2 is **secondary use of clinical text for internal evaluation** → a recorded **DPO note** (same one-line class). The re-translation and generation reuse the **same external processor (OpenAI)** the live path already uses — **no new data-processor exposure** — and all outputs land in the same RLS'd `shadow_register_eval`.
