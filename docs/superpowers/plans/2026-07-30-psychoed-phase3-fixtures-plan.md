...
- **Gate fixtures full-graph (BINDING, ruled):** F4/F6/F8 CI fixtures run the compiled graph end-to-end incl. the escalation-turn audit row; intent is SWEPT (parametrized over `general_chat`, `info_request`, `skill_request`, `emotional_support`), never single-pinned. The flip-tier runner uses REAL intent_route (live LLM, on-demand, prod parity) — that run, not CI, satisfies "F1–F10 green full-graph at prod parity" (spec §7.3).
