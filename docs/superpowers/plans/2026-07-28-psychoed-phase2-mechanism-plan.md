...
- **State keys: exactly 12** — the 10 spec §4.2 keys + `psychoed_family_exposures` (Task 9 rationale; schema-extension follow-up to spec §10 item 7) + `psychoed_weave_escalation` (Task 8; escalation routing signal). All declared in `SageState`; `scripts/check_state_channels.py` must stay green (CI: `.github/workflows/unit-gate.yml:80`).
- **Pathway-clear-on-exit is implemented code, not an emergent property** (spec §4.2): skill-activation exit clears in `skill_select` (Task 8), escalation exit clears in `crisis_response` AFTER its audit write (Task 8/11 — persist-before-clear), and Task 13's graph test asserts both.
- **The escalation turn MUST reach `session_audit`** (Rider 1 / spec §6.3 at the single most clinically important turn): `crisis_response` already calls `write_session_audit({**state, ...})` (graph.py:76–87 precedent) — the psychoed fields flow through it once Task 11 adds them to the audit row builder; the escalation turn's row carries `psychoed_weave_state="escalated"` + the matched-row facts, asserted in Task 13.
...
Helpers added in the same file: `_psychoed_grief_context(state) -> bool` — deterministic signals in this order, LEADING with the one populatable today: recent `grief_loss` in `offered_skill_ids` or `active_skill_id == "grief_loss"` (populated by Mechanism-A consult since 2026-07-23), OR `state.get("psychoed_active_category") == "s2c"`; a clinical-flag signal may be ADDED only after verifying the flag actually exists in the deployed flag set (`rules/data/safety/clinical_flag_patterns.json`) — do NOT reference a nonexistent `"grief"` flag; if none exists, ship with the two live signals and note it in the as-built section. Also `store_manifest_weave(cat)` (reads `store.manifest(cat)["safety_weave"]`). `psychoed_weave_escalation: bool` becomes a declared channel (add to state.py in this task, keep channel-check green).

**Pathway-clear-on-exit (implementing logic, this task):**
- *Skill-activation exit:* in `skill_select_node`, when any NON-psychoed path activates a skill (`_resolve_entry`/consult/post-crisis returns with `active_skill_id` set) while `state.get("psychoed_active_category")` is set, merge into that return: `{"psychoed_active_category": None, "psychoed_delivery_shape": None, "psychoed_menu_offered": False, "psychoed_weave_fired": False, "psychoed_weave_pending": False, "psychoed_matched_row_id": None, "psychoed_collision_path": None, "psychoed_framing": None}` (`psychoed_blocks_served` and `psychoed_family_exposures` survive — audit/carry-forward). Implement as a small helper `_psychoed_pathway_clear(state) -> dict` returning `{}` when no pathway active, spread into those returns.
- *Escalation exit:* `crisis_response` (graph.py `_crisis_response_node`) — when `state.get("psychoed_weave_escalation")`: FIRST its existing `write_session_audit({**state, ...})` call runs with the psychoed facts intact and `psychoed_weave_state="escalated"` patched in, THEN its return dict spreads `_psychoed_pathway_clear(state)` plus `psychoed_weave_escalation: False`. Persist-before-clear, at the code level, in that order.

Graph changes: `_route_after_skill_select` gains, at TOP priority, `if state.get("psychoed_weave_escalation"): return "crisis_response"` and, after the containment check, `if state.get("psychoed_serve") or state.get("skill_match_method") == "psychoed_menu_after_weave": return "knowledge_retrieve"`; edge map at graph.py:452 gains `"crisis_response": "crisis_response"`.
...
- **Escalation-turn audit (gap-2 fix):** `audit.py::_build_session_audit_row` extracts the six psychoed fields from WHATEVER state dict it receives — which makes the crisis node's existing `write_session_audit({**state, ...})` call carry them automatically on escalation turns; add `psychoed_weave_state="escalated"` to the crisis node's inline patch when `psychoed_weave_escalation` is set (Task 8 owns the crisis-node edit; this task owns the row builder). Test (this task): a state dict with psychoed keys + the crisis-node patch shape → row contains all six fields with `psychoed_weave_state="escalated"`.
...
- [ ] **Step 1:** Graph-level test (mocked LLM + no-DB fixture pattern from `tests/conftest.py::asgi_client`): flag ON `{"3c"}`, one full `app.ainvoke` turn with "Why do I feel numb?" → response text contains the 3c framing + 3c-b4 content + weave question, no menu; state carries `psychoed_weave_pending=True`; audit log dict (captured via caplog or the audit task patch pattern used in `tests/test_output_gate_audit.py`) carries `psychoed_matched_row_id="3c-t3"`. Second turn "kind of" → routed to `crisis_response`; assert (a) the crisis card path fired, (b) **the escalation turn's audit row exists and carries `psychoed_weave_state="escalated"` + `psychoed_matched_row_id`** (gap-2 assertion — capture the crisis node's `write_session_audit` call), and (c) **pathway-scoped keys are cleared in the post-turn state while `psychoed_blocks_served`/`psychoed_family_exposures` survive** (gap-1 assertion). Third assertion set: a fresh 3c trigger after escalation-clear re-arms the weave (`psychoed_weave_fired` was reset).
- [ ] **Step 2:** Append to the handoff notes: the as-built deltas — `psychoed_family_exposures` channel (replaces the dead `techniques_used` read; seam finding), `psychoed_weave_escalation` channel + the one edge-map addition, the §6.2 fallback refinement (manifest `check_in` substitute, rationale), the Mechanism-A coexistence semantics (per-category, retirement still flip-time), and the grief-context signal set actually shipped. **Also update the spec's §10 register in the same commit** (`docs/superpowers/specs/2026-07-23-psychoeducation-pathways-design.md`): add as-built amendments — the skill_select→crisis_response edge-map delta and the `psychoed_family_exposures` channel substitution — as numbered register entries, not handoff-notes-only.
