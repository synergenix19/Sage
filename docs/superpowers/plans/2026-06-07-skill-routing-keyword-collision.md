...
Add to test suite. Notes on correctness:

- `_tier1_match` must import `SKILL_REGISTRY` explicitly and iterate in that order — both because it makes the scan order intent explicit, and because the production matcher at `skill_select.py:137` iterates `_SKILLS.items()` which is built from `SKILL_REGISTRY` insertion order. Using dict iteration is equivalent today but breaks silently if `_SKILLS` initialization ever changes.
- Substring-shadowing (the dominant failure mode causing 20 of the 26 collisions) is not caught by an exact-duplicate check. A separate test documents the known 20 and rejects any new addition.
...
def test_no_cross_skill_keyword_duplicates():
    ...
    This test will fail until the 6 pre-existing duplicates are resolved (see pre-existing
    collision table). Mark xfail until the pre-existing cleanup PR ships, then promote to
    a hard gate."""
...
# If an entry is resolved, remove it here. If a new entry is added, the test fails.
...
**Note on `test_no_cross_skill_keyword_duplicates`:** will fail today because of the 6 pre-existing exact duplicates. Mark `xfail` until the pre-existing cleanup PR ships, then promote to a hard gate.
