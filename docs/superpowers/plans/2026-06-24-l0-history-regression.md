...
- **MERGED TO MASTER (2026-06-24, PR #64, commit c33d12d).** Followed the team's actual, evidenced process — identical to PR#51 (the prior clinical L0 change): conventional-prefix branch → PR → **Safety-surface unit tests + Ferry both SUCCESS** → admin-merge (bypassing ONLY the required-review, which is structurally unsatisfiable for a sole-account org; the safety CI passed on merits, not bypassed). The plan's staging→QA→prod step was aspirational and has never been run by any branch (it can't — see Deploy-isolation gap); holding this branch to it would have been inconsistent with PR#51. master now has L0 v2.4.0 (approved, Rohan Sarda) + the eviction fix.
- **PROD DEPLOY:** master-merge auto-triggers the Railway prod deploy per the established CD pattern (PR#51 merged→prod same morning). **NOT verifiable from this session — the Railway token expired (`railway login` needed).** A human must confirm the prod deploy succeeded (Railway dashboard/CLI) and smoke-check live.
- **DEPLOYED + VERIFIED LIVE (2026-06-24 ~23:15):** `railway up` → prod deploy `801691ba` SUCCESS (active); `/health/ready` ready after warmup. **Behavioral smoke test on prod (proper STATEFUL two-turn flow): 3/3 accurate recall** of the DV-adjacent disclosure ("You mentioned that things at home with your husband have gotten scary…") — no false denial, no confabulation. Core fix (vector 2) confirmed working in production.
  - **Methodology note:** prod `/chat` is checkpointer-STATEFUL (`_build_state` takes only `messages[-1]`; `conversation_history` comes from the LangGraph checkpoint). A first single-call test (all turns in one request to a fresh session) was INVALID — the disclosure never entered context — and produced confabulations; corrected to two-turn-per-session, which passed 3/3.
  - **RECONCILED (corrects an over-escalation): the "4/5 confabulate" absent number was an ARTIFACT of the same invalid 3-msg single-call test** (discarded for the present side; I wrongly carried it into a stop-the-line). **Clean absent test through the prod path (single recall, no history): 4 confabulations in 33 trials ≈ 12%, 95% CI ~2–25%** (batches: 1/5, 2/8, 1/20 — high variance, wide band; do NOT quote a pinned figure). NOT a 4/5 regression. Number was over-quoted as 23% off n=13, then ~12% at n=33 — same "don't outrun the sample" lesson as the prevalence "0".
  - **REAL finding (calibrated): the gate OVERSTATED A4-preservation.** Gate = 0% confab (5/5 admit); clean prod ≈ 12% confab (CI ~2–25%). Model identical (gpt-4o). Root cause = **gate methodology**: the gate called `freeflow_respond_node` DIRECTLY (hardcoded `primary_intent`, monkeypatched `_get_prior_context`, no `intent_route`/`skill_select`/`output_gate`), so it measured freeflow in isolation, not the production path. The clinical sign-off rested on a 0%-confab gate; the real prod rate is ~12% (even the low end of the band is above this domain's near-zero-fail-safe bar). Rohan owns acceptable-live-vs-fix/rollback. New-vs-pre-existing unestablished (v2.3.0's blunter wording may have admitted more; not measurable in prod now).
  - **Methodology lesson → all gap tickets:** behavioral gates must exercise the FULL graph path (stateful, real routing), not direct node calls with hand-built state. Both my invalid smoke test AND the gate failed the same way — not modeling prod's stateful full-path reality. This is plausibly the root of every gate-vs-prod gap in this plan; the present-side 3/3 prod check is the only END-TO-END-verified claim.

### Absent-side confabulation — DIAGNOSIS (2026-06-25) and fix order

**Severity: BAD, not borderline.** For memory-of-disclosure on this population the tolerable bar is near-zero with every residual failing SAFE (admit), not unsafe (fabricate). A confabulation asserts something a vulnerable user never said about their own trauma/DV — the invalidation dimension pointed the other way. Quote the rate WITH its sample/CI, not as a pinned "23%".

**Root cause CONFIRMED in code (candidate b — structural absence-signal gap):** the absent case gives the model an *instruction* but no *anchor*. `freeflow_respond.py:112` injects prior-context only `if prior_context:` — **when empty, nothing is injected (silence, no sentinel)**; there is **no "retrieval ran, found nothing" signal** anywhere in the memory path. The KNOWLEDGE path already has exactly this pattern (`composer.py:354`: "No relevant clinical evidence found… Do not fabricate"); the memory path is missing its equivalent. This is the original A4 RCA's own flagged gap ("empty retrieval → block silently omitted") — the recommended fix was "L0 instruction + non-empty sentinel"; **only the instruction shipped (PR#51), the sentinel never did.** With no anchor and a recall question that presupposes a disclosure, the model fills the silence ~1-in-4.

**Fix order (diagnosis-before-fix; engineering-first because the highest-leverage fix is NOT the signed clause):**
1. **Composer empty-retrieval sentinel (highest leverage, engineering, no Rohan sign-off needed — it adds a signal, not clause wording):** when self_reference fired and grounding is genuinely empty (no prior-context, no relevant L1), inject an explicit "no earlier record found for this" sentinel, mirroring the knowledge path. Gives the model something to admit *against*.
2. **Deterministic backstop (if sentinel+wording don't reach near-zero-fail-safe — candidate c):** when self_reference fired AND grounding empty, constrain to a templated admit ("I don't have a record of that, can you tell me again?") instead of free generation. Architecture-consistent (deterministic guardrail over hoping the LLM complies) — likely warranted for absent-history recall on flagged topics.
3. **Clause wording (candidate a, secondary, signed → Rohan):** the absent-side instruction may be under-weighted in a 660-word persona, but the sentinel does the structural work; tune only if needed.

**HARD RULE for the next fix:** validate it through the FULL STATEFUL GRAPH (prod-path), never an isolated node call — fix the gate before trusting the next gate, or it certifies another green number prod contradicts.
- **Carried as follow-ups (findings, not blockers):** deploy-isolation gap, process-gap (signed change without firing test), broader-bloat eviction ticket, and the **absent-side A4 weakness** (now EN+AR, prod-confirmed) → Rohan FYI.
...
3. **The L0 change is PROPOSED-AND-GATED, not shipped.** "Implementation complete" ≠ "deployed": L0 v2.4.0 is `pending_clinical_signoff`, the deploy gate blocks, and the clause wording is still the clinician's to revise. No one should read this trail as "the fix is live." *(Superseded 2026-06-25: v2.4.0 shipped, regressed A4 absent-side live, sentinel fix built+shipping — see #4.)*
4. **THE DURABLE FINDING is the two structural gaps, not the sentinel.** v2.4.0 (our vector-2 fix) introduced the absent-side A4 confabulation and it reached real trauma/DV users **unmeasured** because (a) there is **no isolated staging** to rehearse a deploy (staging shares prod Supabase, unwired — deploy went straight to prod when staging was unrunnable) and (b) the behavioral gate was **N=5 on a mock path** (passed 5/5 by luck). The sentinel fixes the *instance*; **these two gaps are the *mechanism* and are STILL OPEN.** They are NOT lower-priority tail — for a clinical-adjacent system, "we cannot safely rehearse a deploy before it hits real users" is central. Owners required: **minimum-N gate floor** (Process gap §) + **a real isolated staging stage** (Deploy-isolation gap §). The next signed change can fail the same way until both are closed.
...
## Deploy-isolation gap (FLAG — needs an owner; blocked the plan's staging QA step)

**HEADLINE EVIDENCE (2026-06-25) — the Safety-surface CI gate is the only real pre-prod gate, and it WORKED.** On the sentinel PR, the `Safety-surface unit tests` CI caught a genuine integration break the local tree had hidden: the branch (cut from a pre-merge base carrying an unmerged perf commit `3911171`) would have (a) shipped a **broken import** to prod — `freeflow` imported `resilient_message_invoke`, which master no longer defines (`ImportError` on collection of `test_nodes`/`test_routing`), and (b) **smuggled an unrelated perf/checkpointer commit into prod through the sentinel PR** (scope-creep). The gate blocked the admin-merge; the fix was a clean diff-replay onto current master. **This is exactly the deploy-rehearsal function this gap is about — except it fired at CI, not at a staging deploy (which does not exist).** It is direct evidence FOR the two structural findings: had this been a prompt/data change that the safety-unit suite doesn't cover, nothing between "approved" and "serving users" would have caught it. The CI gate is load-bearing and is doing staging's job by accident; a real isolated staging stage + a minimum-N behavioral gate are what make this not luck.

The plan requires a **staging → QA(EN+AR) → prod** sequence, and that staging gate is **unrunnable** as built — two infrastructure facts, same class as the process gap (a clinical-adjacent system with no safe rehearsal stage):
1. **Staging auto-deploy is not wired** — there is no accessible, non-interactive path to deploy a branch to staging, so the artifact can't be put on a staging environment to test it there.
2. **Staging shares the production Supabase** — so even a "staging" smoke test writes to the **production** database; the environment boundary is partly illusory.
2b. **Post-deploy warmup race (observed 2026-06-25 on the sentinel deploy):** after `railway up` SUCCESS, some live `/chat` requests returned `503 "warming up — BGE-M3 index not ready"` *even though* `/health/ready` reported ready — i.e. a window where real users hit warmup with no rehearsal surface to absorb it. Affects ANY deploy (including a rollback), not the sentinel. With no staging, every prod deploy has this live-user warmup window; it also produced a false-alarm "FAIL" in the deploy verify (warmup 503s misread as confabulations). Owner item: make readiness gate the whole instance before serving, or drain/warm before cutover. This is a *reason the staging gate can't be trusted as an isolated test surface*, NOT a license to skip to prod ("staging isn't isolated anyway" is the inversion to avoid — the correct response is fix/flag the isolation, not deploy direct).

**Consequence for this release:** the staging QA gate was **skipped because it could not be run**, not because it was judged unnecessary. The honest status is: *fix built, clinically approved (Rohan Sarda), fully validated at component level (suite 2104, gate 5/5×3, bilingual QA 4/4), BLOCKED on a staging environment that does not exist in deployable-and-isolated form.* Resolving that blocker is a human's call, not something to substitute past.

**The prod-promotion decision is therefore a human risk-acceptance, made in the open — two real options:**
- (a) A human provisions/authorizes a real, isolated staging run (human-owned infra action, not guessed non-interactively), then QA there before merge; or
- (b) A human explicitly accepts component-level validation as sufficient for this POC and authorizes the prod deploy **with eyes open that the staging gate was skipped because it was unrunnable**. For a POC with this validation + clinician sign-off this may be reasonable — but it is a deliberate, named risk-acceptance, not a default backed into.

**Relay distinction to protect:** "we validated thoroughly and a human chose to ship without staging" vs "the merge happened because staging wasn't reachable" — same git command, opposite postures (deliberate vs accidental). This session held the deliberate posture; the merge must stay a human's deliberate act.
...
**DURABLE LESSON (2026-06-25) — minimum N for behavioral sign-off gates.** The thing that let A4 regress past its own gate was not only path (isolated vs full-graph) — it was **N=5**. At a true ~15–25% failure rate, 5 seeds come back all-clean ~30–50% of the time by luck, so the gate's "5/5" was a coin-flip that landed clean, not a measurement. **Every small-N green in a clinical behavioral gate is noise dressed as a number, regardless of path.** Process rule for the owner: behavioral sign-off gates on safety/clinical properties have a **minimum-N floor (n≥30 for certification; small-N runs are iteration signals, never sign-off evidence)**, and run the **full stateful graph**, not isolated node calls. This generalizes beyond this fix — it retroactively makes every N=5 green in this effort suspect; only the full-path prod check is trusted.
