...
**Status:** PLAN APPROVED (PO, 2026-07-08) + **centralisation SHIPPED** (cdai #179, sage-poc #181 — single source + consistency guard). **PO directed serving + centralising `800 46342` / `24/7`** — this is **approval-of-USE, NOT verification.** The G8 correctness questions are **UNRESOLVED and owned by a DIAL-TEST**, not by this approval: (a) is `800 46342` a transcription error for `800 4673` (LifeLine / 800-HOPE, what the directories list)? (b) is the line really `24/7`, or ~8am–8pm? Centralisation makes the eventual correction a one-place edit; it does **not** settle correctness. Do not read "approved" as "correct/final".
...
## Approval record
- **Number:** `800 46342` — PO directed we **keep and centralise** the currently-served value (2026-07-08). **Approval-of-USE, NOT verification** — it does **not** resolve or overrule the G8 transcription question. **Correctness UNRESOLVED, pending a dial-test** (which may confirm `800 4673`). Do not read this as "the number is correct."
