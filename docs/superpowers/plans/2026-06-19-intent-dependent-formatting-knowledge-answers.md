...
- [ ] **T6 — Deterministic output_gate format-strip (COMMITTED FAST-FOLLOW, owner: ⚠️ UNASSIGNED — gate condition NOT met until a name is recorded here).** Move the "no bold/asterisk/em-dash/emoji" guarantee from prompt-trust to a deterministic `output_gate` transform on the live response, aligning with v7 §5.5/§5.6.2. Strip `***`/`**`/`*`/`—`→`, `/emoji while **preserving** newlines and numbered/hyphen lists. Tests:
  ...
  This closes the §2 deviation; it is a fast-follow, not optional, and must land before prod merge-enable of the structure permission.
...
- **Model adds `**bold**`/emoji given list permission, and it reaches the user un-stripped (§2).** Mitigation: directive forbids them (probabilistic) **and T6 deterministic output_gate strip (committed fast-follow, must land before prod merge-enable)** makes the guarantee deterministic per v7 §5.5. Confirmed live that Abby does not self-forbid these even in Arabic, so prompt-trust alone is insufficient. **Low once T6 lands.**
...
2. ⚠️ **T6 deterministic-strip owner — UNASSIGNED.** "Committed with a named owner" was the condition; an unnamed committed task is the thing most likely to silently slip after the permission ships. Plus T6 now requires the **false-positive guard (test iv)** so the strip cannot corrupt cited `*`/`—`.
