...
**Goal:** Restore TIPP reachability at High intensity — a drive at `ei=8` currently resolves to `box_breathing`, not `dbt_tipp` — and couple that restoration **fail-closed** to the SG-2 cardiac/pregnancy caveat, so TIPP is not routable unless the contraindication screen is present.
