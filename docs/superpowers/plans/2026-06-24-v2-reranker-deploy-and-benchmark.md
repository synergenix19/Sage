...
2. **PROD HEALTHCHECK TIMEOUT (config check before prod).** Two-model warmup observed at **~4.5 min** on Railway CPU (503 → 200), which EXCEEDS the doc's earlier "timeout >= 120s". Before prod: confirm the Railway healthcheck timeout (if a healthcheck is configured) is **≥ ~300s**, else a too-short timeout marks a still-warming deploy as failed. Staging deploy-success was NOT gated on `/health/ready` (SUCCESS at 14:52 while ready at 14:57; old container served during warmup, zero-downtime).
