...
| G2 | **Crisis recall — MEASURED 2026-06-24 (composite, full graph).** ~38% was **S1-alone**; composite **English** ≈ 69% (active-SI 94% / passive-SI 82% / **self-harm ~40%, weak**), FP ~6%. **⛔ Oblique-KHALEEJI (the pilot population) = UNMEASURED — CRADLE is English-only.** Report **per-cell**, never "38%" or "69%". **No posture relaxation:** the cell that matters is unmeasured; English uplift is input to a human bar decision, not a relaxation. Needs an **Arabic crisis bench** (task #21; the 3 new AR cases are its seed). Detail: `2026-06-24-composite-crisis-recall-measurement-PROPOSAL.md`. | EN measured; **AR unmeasured (no bench)**; self-harm weak; S2 unbuilt | **clinical** (bar, per-cell) + engineering (Arabic bench + measure + S2) |
