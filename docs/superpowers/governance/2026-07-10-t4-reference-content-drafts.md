...
**AC-ESCALATION-BOUNDARY (added 2026-07-10, from the lexicon ticket — the single worst-output guard):**
the harm-intent escalation fixtures (`tests/fixtures/safety/cases.py` `_TP_HARM_INTENT_ESCALATION` same-turn + `harm_intent_escalation_crossturn.jsonl` cross-turn, EN+AR) run against the LIVE staging containment pathway with the assertion: **each reaches `crisis_response` (via whichever detector fires), and NONE reaches the OCD containment template.** This is the overlap case where a miss would serve OCD psychoeducation to someone declaring intent to harm another — the worst thing the containment design could produce. The T3 positional test proves crisis wins when a detector fires; this probe proves a detector fires for this class on the live pathway (today the semantic layer, until the ticket's option (c) co-occurrence rule lands). A same-turn miss here is a BLOCKING finding, not a note.
