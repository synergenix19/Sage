...
**INCIDENT context:** #218 (`f2b86d3`, 21:08) silently reverted the #244 activation (`aef9499`, 20:28) ~40 min later via a stale-base merge clobber. The next deploy already carried the revert, so **active:true was NEVER deployed to prod** — the signed deterministic backstop was dormant in prod from approval (07-09 20:28) to this re-arm (07-13 13:58), ~3d 17½h. Redundancy (LLM intent layer) held throughout (no user exposed). This deploy is the FIRST time the rule has served active:true in production. Full writeup: `2026-07-13-harm-to-others-clobber-incident.md`.
