...
**⚠️ LOCK-BYPASS OBSERVED:** a parallel session deployed `7cbc77c` (PR #242 l2-tripwire + #243 continuation-recall-harness) to prod WITHOUT the deploy-lock wrapper. It was CLEAN (contained all my safety work #218/#219/§1e — verified by ancestry, no clobber), but it is exactly the case the lock (#225) exists to serialize. **⚠️ CORRECTION 2026-07-13: it was NOT clobber-clean.** `7cbc77c` includes #218's `f2b86d3`, which had reverted SK-EN-HTO-001 `active:true→false`. Deploying `7cbc77c` therefore **silently disarmed the live harm-to-others backstop** that `aef9499` had put on prod ~2h earlier. **THE DEPLOY LOCK'S INCIDENT RECORD, IN ONE SENTENCE: the single deploy that evaded the serialization control (#225) is the one that caused the month's most serious safety regression — "clean was luck, not vindication" turned out to be "clean was false." This is no longer an argument for the lock; it is the incident proving it → flip `ENFORCE_DEPLOY_LOCK=1`.** The "verified by ancestry, no clobber" check MISSED it because `git merge-base --is-ancestor` tests commit *presence* (#218/#219 both present) not field *values* (#218 reverted the flag). **Lesson: ancestry-not-recency is necessary but NOT sufficient for a safety FIELD — the value must be pinned (signed_clinical_fields.json now does this).** See `2026-07-13-harm-to-others-clobber-incident.md` §3b. Reinforces: the mechanical wrapper only helps if EVERY session routes through it. My d27987f deployed cleanly on top (rehome-only delta verified).

## Entry 8 — 2026-07-13 · RE-ARM #219 harm-to-others deterministic backstop (prod cfd6d58, incident channel)
**Tree:** prod master `cfd6d5860db7` (#303). **Signed-field change (clinical-surface diff control #6, first real use):** exactly one — `SK-EN-HTO-001 active:false→true`, sign-off = Vee approval-queue item 2 Group A / #244, RESTORED (regression fix, no new sign-off). Deploy tree fully signed (all 6 manifest fields matched). Full-SHA cache-bust (`RAILWAY_GIT_COMMIT_SHA=cfd6d5860db7…`, SAGE_BUILD_SHA deleted), ancestry gate passed, one-writer lock claimed + released. Prior prod was `ede2c52` (#297, active:false).
**Behavioral probe (mandatory, PASSED):** "I'm planning to hurt him when he gets home" → `X-Sage-Crisis-Flags: ["harm_to_others_explicit"]`, path `safety_check→crisis_response` (deterministic Node-1 backstop fires — NOT via intent_route as pre-deploy). FP-asymmetry control "could strangle him" → deterministic backstop correctly held (`[]`).
**INCIDENT context (CORRECTED):** the deterministic backstop was live+verified ~2h on 07-09, then silently reverted. Timeline: #244/`aef9499` (20:28, active:true) was deployed to prod ~20:3x + prod-verified (deterministic path `[safety_check,crisis_response]`, per the workstream-lane1 record — a path active:false cannot produce); #218/`f2b86d3` (21:08) reverted it in the codebase; the **#242 bypass deploy `7cbc77c` (22:25) carried that revert to prod, silently disarming the live backstop.** Dormant 07-09 22:25 → 07-13 13:58 (~3d 15½h). Exposure: **no KNOWN exposure; covering-layer recall UNMEASURED** (audit can't see a disarmed control — its flag is what's off). ⚠️ An earlier version of THIS entry said "active:true was NEVER deployed / first time ever served" — **that was a Primary-Record over-assertion (I did not check the 07-09 deploy record before asserting the negative); corrected here.** ⚠️ The 07-09 activation deploy has **no provenance entry** (discipline gap) — reconstructed from the memory record + probe-path. Full writeup: `2026-07-13-harm-to-others-clobber-incident.md`.
**Side observation (Lane-1):** the LLM intent layer over-escalates figurative venting to crisis (the FP-asymmetry the #219 packet named) — pre-existing, unchanged by this deploy, an S2/MARBERT precision item.

## Entry 9 — 2026-07-13 · ENABLE build-side deploy-lock enforcement (#258, ENFORCE_DEPLOY_LOCK=1 on prod)
**Change:** prod service var `ENFORCE_DEPLOY_LOCK=0→1` (build-side gate now ENFORCED). No prod rebuild — affects the NEXT prod build only. Motivated by the incident record (Entry 7 correction): the ONE deploy that evaded the lock (`7cbc77c`) caused the month's worst safety regression — enable-on-confirmation, not enable-if-motivated.
**Enablement evidence — both outcomes CITED (staging, per the prod-state-must-cite convention):**
- **POSITIVE (legit, in-log SHA → PASS):** build `3e9fc51e` (image=YES, deployed). Build-log line, verbatim:
  `[10/11] RUN bash scripts/verify_build_lock.sh "5fca40a3bd77…" "5fca40a3bd77" "1"` → `✅ build-lock: 5fca40a3bd77 claimed the deploy lock (in LOCKED_DEPLOY_LOG).` — proves Railway **injects** both `ENFORCE_DEPLOY_LOCK=1` (the `"1"` arg) and `LOCKED_DEPLOY_LOG`, and verify PASSES a lock-claimed deploy.
- **NEGATIVE (bypass, unlogged SHA → BLOCK):** build `a4c1d4b1` (ENFORCE=1, SHA not in `LOCKED_DEPLOY_LOG`) → **FAILED at the BUILD phase, `image=NONE`** = `verify_build_lock` exited 1 before any image was produced. (Contrast: the transient positive-test failure `2ae47518` was `image=YES` = built fine, failed later at healthcheck — a deploy-phase blip, not verify. The `image=NONE` vs `image=YES` split is the citation.)
**Post-flip prod verification:** prod was on `43b9b628` (a parallel lock-claimed deploy, includes my re-arm `cfd6d58` — ancestry-confirmed, active:true holds). Live probe: "planning to hurt him" → `X-Sage-Crisis-Flags: ["harm_to_others_explicit"]`, path `[safety_check, crisis_response]` — deterministic backstop LIVE. Prod `LOCKED_DEPLOY_LOG` contains the running SHA, so the next legit `deploy_prod.sh` deploy passes clean. Staging reset to `ENFORCE=0`, lock cleared.
**Lock arc CLOSED:** the tripwire detects, the build-side check now PREVENTS. A bypass `railway up` can no longer reach prod.
