...
- [x] **Deprecate** — removed `mi_readiness_ruler` from `SKILL_REGISTRY` + its clinical cluster + its offer description. **Mechanic corrected:** the original "JSON stays in the repo" was wrong for this repo — `test_corpus_integrity.py::test_no_orphan_skill_jsons` forbids unregistered skill JSONs, so the JSON was **deleted**; reversibility is via **git history** (`git revert` / restore the file), still cheap and fully reversible. Freed slot taken by `mindfulness_meditation`; routing verified (0 new failures, calibration gap 0.0526 clean, `SEMANTIC_THRESHOLD` unchanged).
