...
| 4 | **Intensity classifier confusion at the 6–7 boundary** — *direction*, not generic A/B: how often, and which way, does it confuse 6 and 7? | **HARNESS BUILT** (`scripts/d5_intensity_confusion_probe.py`, raw: `d5-confusion-probe-provisional.txt`). **Provisional run (labels NOT clinician-validated, do not treat as a result):** classifier did **not under-rate** acuity on the seed — intended-7 came back 7-8 (0/5 under-rated), intended-6 mostly 5-6 (1/5 over-rated). *Direction mildly favors floor 7* (a true 7 isn't missed as 6). **Still OPEN: needs a clinician-validated labeled set before it informs the pin.** |
