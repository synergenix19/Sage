...
**STANDING CONVENTION (subagent lifecycle) — established 2026-07-08:** a subagent's workspace (worktree, scratch files) is torn down **only after its deliverable is verified harvested** — commits confirmed on the branch, tests re-run, artifacts read — **never on a stop/lifecycle signal alone.** An agent stop-notification is a *lifecycle* event, not a *completion* signal. The same root — treating lifecycle signals as completion — caused both a stale re-notification acted on twice *and* a premature worktree cleanup (torn down while the agent's final suite run was still in flight) in one window. The commits survived both times only because they were pushed to origin and independently re-verified — that is the recovery, not the license. **Verify-then-clean: read "agent stopped" as "go check," never as "done."**

**STANDING CONVENTION (gender inference — bias safety) — established 2026-07-08:** the user's gender is **never inferred from topic, name, or message content**, anywhere in the system. Address may be gendered **only** by (a) explicit grammatical self-marking in the user's own message, or (b) the profile's `cultural_preferences.gender_address` field (Full Build). Absent either, generation uses gender-neutral constructions — *a wrong gender guess is worse than neutrality.* Inferring gender from context (a relationship topic → female, a name → a gender) is a **prohibited bias failure**, not a helpful default. Applies to shadow/serving Arabic generation, skill content, cultural rules, and any future personalization. The deterministic detector `detect_gender_marking` is conservative by design (none-on-doubt) with a first-person-anchor guard that **reduces** the third-party false-positive (a marker counts only with a nearby first-person anchor, and is rejected when a third-person possessor precedes it — *أختي تعبانة* → `none`); the residual is then quantified over the run (reduce, then quantify). Anchor/possessor lists remain starter sets pending Gulf-native linguist review.

**STANDING CONVENTION (gated execution — evidence, not readiness) — established 2026-07-09:** a gated action executes **only when its gate's *evidence* exists** — the signature, the attestation, the applied migration, the passing check — **not** when the *machinery* to perform it is ready. "The pipeline was ready," "the version string read `1.0.0`," "the code is merged" are *readiness*, not the gate's evidence. **Second near-instance of this class:** the version-bump discipline HELD 2026-07-08 (refused to bump exemplars off `-draft` without content), then did NOT hold 2026-07-09 — a run labelled "Layer 1" fired on exemplars whose clinician attestation, `[AI-DRAFT]`-string correction pass, and encoding check **did not exist**, because the plumbing was ready and the version string read `1.0.0`. Remediation: reclassified to a **plumbing shakedown**, its 25 rows quarantined out of the pre-registered analysis via provenance columns, the false `1.0.0` reverted to `-unattested`. Honest post-hoc reporting (refusing to fabricate the register number; listing the attestation as outstanding) made it a reclassification, not a trust rupture — **but the gate is the evidence, and readiness is not it.** Pairs with the compliance-class-closure convention (a ticket closes only against enumerated evidence).
