...
### Clinical Note: monitoring → resolved transition requires skill completion

The RESOLVED transition depends on `post_crisis_check_in` reaching `bridge_or_close` AND
the user's message meeting completion criteria (>10 words, or emotional_intensity <= 7).
A user sending very short messages may hold at `acknowledge_and_check` for multiple turns.

This is safe: the skill keeps activating on each turn, S7 keeps classifying, and
heightened-sensitivity prompt framing remains active. A user who disengages entirely
stays in MONITORING for the session duration — the L2 escalation entry
("STILL_DISTRESSED for 3+ turns → flag_for_review") provides a catch.

In practice, the full lifecycle takes 3–4 turns past the initial crisis response rather
than the illustrative 2-step description in the spec. The 5-turn total in the plan was
aspirational; 6 turns is the observed baseline when T3 holds due to a short recovery
message.
